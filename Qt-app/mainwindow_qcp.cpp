    ...
    : QMainWindow(parent)
{
    ...
    // dark background to match Qt dark palette
    QColor plotBg(53, 53, 53);
    QColor rectBg(35, 35, 35);
    plot->setBackground(QBrush(plotBg));
    plot->axisRect()->setBackground(QBrush(rectBg));
    plot->setAntialiasedElements(QCP::aeAll);

    // enable right Y axis for CH1
    plot->yAxis2->setVisible(true);

    // X axis (time) styling
    auto styleXAxis = [](QCPAxis *ax) {
    ...
    // CH0 → left axis (yellow), CH1 → right axis (cyan)
    channels[0] = new Channel(1, Qt::yellow,       plot, plot->yAxis,  awesome, this);
    channels[1] = new Channel(2, QColor("#2FD9D4"), plot, plot->yAxis2, awesome, this);

    mainLayout->addWidget(plot);
    mainLayout->addLayout(controlLayout);
    // ── end QCustomPlot setup ──────────────────────────────────────────────
    ...
    icnPlay     = mkIcon(fa::fa_play,        QColor("#4CAF50"));
    icnStop     = mkIcon(fa::fa_stop,        QColor("#F44336"));
    icnPause    = mkIcon(fa::fa_pause,       QColor("#FFC107"));
    icnRefresh  = mkIcon(fa::fa_sync,        QColor("#40C4FF"));
    icnSave     = mkIcon(fa::fa_save,        QColor("#66BB6A"));
    icnReadFile = mkIcon(fa::fa_folder_open, QColor("#FF9800"));
    ...
    if (port.isEmpty()) {
        ...
        for (auto &portInfo : QSerialPortInfo::availablePorts()) {
            if (portInfo.manufacturer() == "STMicroelectronics" ||
                portInfo.description() == "STM32 Virtual ComPort") {
    ...
    connect(serialReader, &SerialReader::newSamples, this,
            [=](int channel, QVector<double> values, QVector<double> times) {
        if (currentState == stopped) return;
        if (channel < 0 || channel >= NUM_CHS) return;
...
void MainWindow::btnPlayStopHandler() {
    ...
    default:
        ...
        displayMinTime = 0;
        displayMaxTime = 2;
        plot->xAxis->setRange(displayMinTime, displayMaxTime);
        plot->replot(QCustomPlot::rpQueuedReplot);
        btnPauseResume->setEnabled(false);
        btnRefresh->setEnabled(false);
        btnAuto->setEnabled(false);
...
void MainWindow::btnRefreshHandler() {
    displayMaxTime = std::max(
        channels[0]->pts.empty() ? 0.0 : channels[0]->pts.back().x(),
        channels[1]->pts.empty() ? 0.0 : channels[1]->pts.back().x()
    );
    dialTimePos->setValue(displayMaxTime);
    updatePlot();
...
void MainWindow::getFileToSaveTo() {
    QString filePath = QFileDialog::getSaveFileName(
        this, "Choose output file", QDir::homePath(),
        "Text Files (*.txt);;All Files (*)"
    );
    if (!filePath.isEmpty()) {
        QFile *file = new QFile(filePath);
        if (file->open(QIODeviceBase::WriteOnly | QIODevice::Truncate)) {
            if (writeFile != nullptr) writeFile->close();
            writeFile   = file;
...
void MainWindow::getFileToReadFrom() {
    QString filePath = QFileDialog::getOpenFileName(
        this, "Choose input file", QDir::homePath(),
        "Text Files (*.txt);;All Files (*)"
    );
    if (!filePath.isEmpty()) {
        QFile *file = new QFile(filePath);
...
void MainWindow::btnAutoHandler() {
    ...
    if (p1 == INFINITY || !channels[0]->isActive) p1 = 1e-9;
    auto p2 = channels[1]->period;
    if (p2 == INFINITY || !channels[1]->isActive) p2 = 1e-9;
    double timeRng = (p1 > 1e-9 && p2 > 1e-9)
                     ? 2 * std::max(p1, p2)
                     : 4 * std::max(p1, p2);
    dialTimeRng->setValue(timeRng);
    displayMinTime = std::max(0.0, displayMaxTime - dialTimeRng->getValue());
    plot->xAxis->setRangeLower(displayMinTime);
    plot->replot(QCustomPlot::rpQueuedReplot);
}
...
void MainWindow::updatePlot() {
    ...
    channels[0]->updateGraph();
    channels[1]->updateGraph();
    displayMinTime = std::max(0.0, displayMaxTime - dialTimeRng->getValue());
    plot->xAxis->setRange(displayMinTime, displayMaxTime);
    plot->replot(QCustomPlot::rpQueuedReplot);
}
