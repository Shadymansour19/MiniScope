...
Channel::Channel(int id_, const QColor& color_, QCustomPlot *plot, QCPAxis *yAxis,
                 fa::QtAwesome *awesome, QWidget *parentWidget)
    : id(id_), color(color_), isActive(true),
      displayMinY(-1.0), displayMaxY(1.0),
      amplitude(0), dcLevel(0), frequency(0), period(0)
{
    ...
    // graph & axis (yAxis / yAxis2 passed in from mainwindow)
    axis  = yAxis;
    graph = plot->addGraph(plot->xAxis, axis);
    graph->setName(label);
    pen = new QPen(color);
...
// Converts pts (QVector<QPointF>) into the two separate QVector<double> arrays
// that QCPGraph::setData() expects, then pushes to the graph.
void Channel::updateGraph() {
    QVector<double> keys(pts.size()), vals(pts.size());
    for (int i = 0; i < pts.size(); i++) {
        keys[i] = pts[i].x();
        vals[i] = pts[i].y();
    }
    graph->setData(keys, vals, /*alreadySorted=*/true);
}
...
void Channel::OnOffHandler() {
    ...
    btnOnOff->setIcon(isActive ? icnOn : icnOff);
}
...
QString Channel::formatNum(double num) {
    static QStringList factorsText = {"n", "µ", "m", "", "k", "M", "G"};
    static QList<double> factorsVal = {1e-9, 1e-6, 1e-3, 1e0, 1e3, 1e6, 1e9};
    if (num == 0.0) return "0.0 ";
    int i = 0;
    double absNum = std::fabs(num);
    while (i + 1 < factorsVal.length() && absNum / factorsVal[i] >= 1e3) i++;
    return QString::number(num / factorsVal[i], 'f', 1) + " " + factorsText[i];
}
...
void Channel::analyze() {
    ...
    CArray data(N);
    for (int i = 0; i < N; ++i) data[i] = Complex(vals[i], 0.0);
    fft(data);

    std::vector<double> mag(N / 2);
    for (int i = 0; i < N / 2; ++i) mag[i] = std::abs(data[i]);

    std::vector<double> freq(N / 2);
    double fs = 1.0 / ts;
    for (int i = 0; i < N / 2; ++i) freq[i] = i * fs / N;
